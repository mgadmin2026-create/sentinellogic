...
export default function SyncPage() {
  ...
  async function handleTaggedContactsSync(
    ...
    try {
      for (const phase of ['klicktipp', 'superchat'] as const) {
        let cursor: string | null = null
        for (let batch = 0; batch < 500; batch += 1) {
          ...
          if (!json.success) throw new Error(json.error || 'Sonderaktion fehlgeschlagen')
              ...
              <div className="flex flex-col gap-2 mt-auto">
                {source.id === 'superchat' && (
                  <>
                    ...
                    >
                      {sparteFehltSyncLoading ? 'Sparte fehlt wird verarbeitet…' : 'Sparte fehlt → KlickTipp & SuperChat'}
                    </button>
                    <button
