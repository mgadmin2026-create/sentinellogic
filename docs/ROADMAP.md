...
| **UI-/Branding-Überarbeitung** | Hoch | ✅ Done, in `main` gemergt (Stufe 1–3, `docs/STYLE_GUIDE.md`, Stand 2026-08-16) | Fundament, Pilotseiten und alle übrigen Hauptseiten auf `<PageHeader />`/`<Button />` + `brand`-Token umgestellt. Nach Nutzer-Review nachgebessert: einheitlicher Seiten-Container (kein Gradient-Hintergrund/kein `mx-auto` mehr auf ~10 Seiten inkl. Einstellungen-Unterseiten, Testdashboard, Postfach, Kontakt-Detail), Kontakt-Detail rechte Spalte neu sortiert (Erstgespräch → Nächste Aufgabe → Angebote → Telefonie & Sync), Aktivitäten-Timeline mit durchgehender Verbindungslinie statt abreißender Segmente plus Zeitraum-Gruppierung (Heute/Gestern/Diese Woche/Diesen Monat/Monat Jahr), Aufgaben-Tabelle/-Filterleiste an Angebote-Tabelle angeglichen (inkl. neuem `Badge`-Farbton `orange`). Live verifiziert, `feature/ui-ux-fundament` (enthielt gemergt `feature/angebote-tracking`) nach `main` gemergt |
...
| **SuperChat-Integration/Ablösung** | Hoch | 🟡 Providerneutrales Inbox-Grundgerüst auf `feature/zentrale-inbox`: Gesprächsmodell, Verlauf, Zuweisung, Status, interne Notizen und Kontaktsprung | Migration zunächst nichtproduktiv anwenden; danach SuperChat-Gespräche read-only importieren und Webhooks anbinden. Direkten Meta-Adapter erst nach sicherer Abnahme aktivieren |
| **SuperChat-Datenmigration** | Niedrig | 🔴 Nicht umgesetzt | Erst nach strategischer SuperChat-Entscheidung betrachten |
| **Vollständiges E-Mail-Postfach / Unified Inbox** | Hoch | 🟡 `/postfach` bleibt produktiv; kanalübergreifendes Inbox-Grundgerüst unter `/kommunikation` auf dem Feature-Branch vorhanden | Nach WhatsApp-Pilot das STRATO-Postfach über einen E-Mail-Adapter in dieselbe Gesprächsansicht überführen; Anhänge und Threading dabei abnehmen |
...
| **Datenqualitäts-Agent** ("Datenqualitäts-Center") | Niedrig | 🟡 Konzept steht (`docs/DATENQUALITAETS_KONZEPT.md`, Stand 2026-08-16) | Ausgereiftes Konzept: 8 deterministische Erst-Regeln (Qualifiziert ohne Angebot, Kunde ohne Vertrag, Angebot gewonnen ohne Beitragsübersicht-Update, Kontaktiert/Aufgabe/Pipeline-Schritt überfällig, Kontakt ohne Verantwortlichen, Sparte fehlt), Wiederverwendung von `sync_runs` als Ausführungsmodell (Prüfung=Batch, Verstoß=Item) statt neuer Infrastruktur, automatische Aufgaben-Erzeugung über bestehendes `triggered_by_rule`-Feld, Fingerprint-Dedupe gegen doppelte Meldungen. KI bewusst erst als spätere Ausbaustufe (Freitext-Regel-Vorschlag mit Bestätigungspflicht, dann Anomalie-Erkennung über das bestehende Call-Prep-`flags`-Feld), nicht Teil von Version 1. Drei offene Fragen vor Umsetzungsstart siehe Konzeptdokument (Schwellenwert-Konfiguration, Navigations-Standort, MVP-Regelzahl) |
