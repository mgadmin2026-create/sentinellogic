...
| **UI-/Branding-Überarbeitung** | Hoch | 🟡 Konzept steht (`docs/UI_UX_KONZEPT.md`, Stand 2026-08-15) | Konzept mit Ist-Analyse, Design-Tokens, Bausteinkasten und 4-stufigem Rollout abgestimmt: Fundament → Pilotseiten (Dashboard/Kontakte/Angebote) → Rest → `docs/STYLE_GUIDE.md`. Entschieden: Style Guide als Markdown-Datei (kein `/styleguide`-Route), keine `<DataTable />`-Abstraktion, Sidebar-Default bleibt ausgeklappt. Nächster Schritt: Stufe 1 (Design-Tokens + UI-Bausteine + einklappbare Sidebar) umsetzen |
