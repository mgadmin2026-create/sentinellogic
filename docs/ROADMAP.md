...
| **UI-/Branding-Überarbeitung** | Hoch | ✅ Done (Stufe 1–3, `docs/STYLE_GUIDE.md`, Stand 2026-08-16) | Fundament, Pilotseiten und alle übrigen Hauptseiten (Aufgaben, Kalender, Dokumente, KI-Upload, Automatisierungen, Selektion, E-Mail, Hilfe, Einstellungen, Profil, Erwähnungen, Release Notes) auf `<PageHeader />`/`<Button />` umgestellt; projektweiter Hex-Sweep (`#1A1A1A`/`#FFC300`/`#e6b000` → `brand`-Token) über App + Komponenten, live verifiziert (Branch `feature/ui-ux-fundament`, enthält gemergt `feature/angebote-tracking`). Bewusst nicht migriert: Testdashboard-Kopf (abweichende Struktur) und Einstellungen-Unterseiten (nur Typografie vereinheitlicht, nicht auf `<PageHeader />` umgestellt) — beides optional für einen späteren Anlauf |
