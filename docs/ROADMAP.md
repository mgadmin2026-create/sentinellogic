...
| **UI-/Branding-Überarbeitung** | Hoch | 🟡 Stufe 1 (Fundament) done (`docs/STYLE_GUIDE.md`, Stand 2026-08-16) | Design-Tokens (`brand`-Farben in `tailwind.config.ts`), Bausteinkasten `src/components/ui/` (`Button`, `Card`, `Badge`, `PageHeader`, `EmptyState`) und einklappbare Sidebar (`localStorage`, Default ausgeklappt) umgesetzt und live verifiziert (Branch `feature/ui-ux-fundament`). Nächster Schritt: Stufe 2 — Pilotseiten Dashboard/Kontakte/Angebote auf die neuen Bausteine umstellen |
