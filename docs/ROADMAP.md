...
| **UI-/Branding-Überarbeitung** | Hoch | ✅ Done inkl. Nachbesserung (Stufe 1–3, `docs/STYLE_GUIDE.md`, Stand 2026-08-16) | Fundament, Pilotseiten und alle übrigen Hauptseiten auf `<PageHeader />`/`<Button />` + `brand`-Token umgestellt. Nach Nutzer-Review nachgebessert: einheitlicher Seiten-Container (kein Gradient-Hintergrund/kein `mx-auto` mehr auf ~10 Seiten inkl. Einstellungen-Unterseiten, Testdashboard, Postfach, Kontakt-Detail), Kontakt-Detail-Kacheln Angebote/Telefonie & Sync in die rechte „Arbeit"-Spalte verschoben, Aktivitäten-Timeline mit durchgehender Verbindungslinie statt abreißender Segmente, Aufgaben-Tabelle/-Filterleiste an Angebote-Tabelle angeglichen (inkl. neuem `Badge`-Farbton `orange`). Live verifiziert (Branch `feature/ui-ux-fundament`, enthält gemergt `feature/angebote-tracking`) |
